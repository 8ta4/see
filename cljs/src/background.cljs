...
(defn finalize
  [id]
  (promesa/let [results (js/chrome.scripting.executeScript (clj->js {:func getText
                                                                     :target {:tabId id}}))]
...
(defn take-screenshot
  [id]
  (promesa/let [screenshot (js/chrome.tabs.captureVisibleTab)]
...
(defn handle-host
  ...
  (js/console.log url)
  (promesa/let [tab (js/chrome.tabs.create (clj->js {}))]
